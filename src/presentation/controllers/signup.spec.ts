...
describe('SignUp Controller', () => {
  ...
  test('Should return 400 if password is different from passwordConfirmation', async () => {
    const { sut } = makeSut()
  ...
  test('should call AddAccont with correct values', async () => {
  ...
  test('Should return 500 if addAcount throws', async () => {
    const { sut, addAccountStub } = makeSut()
    jest.spyOn(addAccountStub, 'add').mockRejectedValue(() => { throw new ServerError() })
    const httpRequest = {
      body: {
        name: 'any_name',
        email: 'invalid_email@mail.com',
